...
## ¿Y si la víctima no puede conectarse? (inconsciente, atrapada)

Existe una **excepción gobernada** para ese caso (`emergency-autojoin-profile`, ADR-005, RFC-0003): un AP de emergencia responde a cualquier SSID que el teléfono sondee, y el portal cautivo — que iOS/Android abren automáticamente — convierte el dispositivo en baliza (sonido, vibración, flash) con un mensaje de emergencia. Es la acción legalmente más delicada del proyecto (engaño activo al dispositivo), así que sólo opera bajo el carril completo de `emergency_assumed_risk`: doble autorización, geofence exacto, monitoreo, expiración y kill switch; nunca por defecto; sin captura de credenciales, sin rerouting, sin inspección de contenido; y la apertura del portal jamás confirma una persona localizada. La eficacia real es baja y cayendo con los OS modernos: estado `unverified`, con un experimento de medición definido. Para detectar presencia no hace falta: los probes pasivos ya bastan; esto aporta el canal. Ver [guía](guides/emergency-autojoin.md).

## ¿Se conecta con ATAK/SARCOP/CalTopo?
