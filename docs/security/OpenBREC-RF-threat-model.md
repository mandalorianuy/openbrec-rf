...
There is no deployed field application to assess. M0 now provides executable laboratory evidence, including a worker that commits to PostgreSQL before acknowledging, but addon threat ratings still describe proposed architecture and conditional risk. The RF sensing addons (CSI, passive RF, SDR receive-only, drone geometry, RF quieting) are `specified`/`simulated` designs with contract-level safety invariants; their reviews (`rf-sensing-csi-review.md`, `passive-rf-review.md`, `sdr-receive-review.md`, `drone-geometry-review.md`, `rf-quieting-review.md`) are design reviews, not runtime assessments. Before field implementation, every high-risk path needs schemas, fixtures, negative tests, owners and evidence receipts. Implemented deltas and limits are reviewed in the dated M0 security reviews under `docs/security/`.
...
- the RF sensing addon domains reintegrated by ADR-004 — Wi-Fi CSI/radio-tomography observations, RuView as pinned optional provider, passive RF metadata, receive-only SDR profiles, drone deployment events and measured RF isolation profiles — as contract-level designs only; no collectors, firmware or field runtime exist;
- the governed emergency autojoin exception (`emergency-autojoin-profile`, ADR-005): a Karma-style AP answering any probed SSID with a labeled emergency captive portal, allowed only under the full `emergency_assumed_risk` lane; contract design only, efficacy `unverified`.
...
- Planned RF sensing addon surfaces: CSI Tx/Rx links and baselines, passive RF collectors and pseudonymized subject references, SDR receivers and direction-finding arrays, drone payload release workflows and RF isolation assemblies (contract designs only; no runtime exists).
...
| TM-005 | Compromised federation hub forges authority, suppresses events or gains cell secrets. | Hub or operator account compromised; cells over-trust hub data. | Misdirection, delayed handoff, privacy loss and cross-cell compromise. | Hub has no cell keys, verifies but cannot originate cell signatures, two hubs, outbound-only gateways, local autonomy, signed events and reconciliation. | P0-05 malicious-hub simulation and P0-08 integrated 24h hub-loss campaign pass; physical key inventory and failover drill remain planned. Owners: federation + security. | Hub can delay/drop visible traffic and analyze permitted metadata; runtime and physical custody remain unverified. | High |
...
| TM-010 | Split-brain reconciliation silently overwrites handoff, topology or resource history. | Long partition, concurrent decisions and last-write-wins implementation. | Lost task ownership, duplicated deployment or missed response. | Append-only union, signed causal events, monotonic safety states, visible conflict events and authorized resolution. | P0-05 24-hour partition/50k-site replay passes 10 orderings with 15 visible conflicts and zero overwrite/loss; runtime remains planned. Owners: federation + core. | Human resolution may be slow, local views may temporarily diverge and distributed runtime is unverified. | High |
| TM-011 | Adversarial or environmental sound, heat, motion or placement creates or suppresses a beacon candidate. | Physical/RF proximity, unmodeled environment, stale baseline or overconfident model/fusion. | Resource diversion, missed distress, false location and alert fatigue. | Typed observations, health/placement invalidation, deterministic baseline, OOD/abstention, non-independent fusion and mandatory human review. | P0-07 replays 12 observations and 12 hostile cases with one deterministic projection, zero false confirmation/absence and visible OOD/movement/gaps; physical campaign remains P1. Owners: sensing + search operations. | All inputs are synthetic; real incidents exceed test coverage and multiple sensors may share one false cause. | High |
| TM-012 | Raw audio, thermal grids or fine placement data are captured, retained or exported without justified scope. | Compromised operator, broad role, weak retention worker or exposed vault. | Victim/responder privacy harm, surveillance, legal risk and loss of trust. | Features-only default, dual-role authorization, bounded break-glass, encryption, hash references, vault ACL, hold/review and deletion receipts. | P0-07 retention-fault reconciles 7/7 modeled cases, traces 4/4 materials and rejects unauthorized/expired capture; runtime vault and physical custody remain unverified. Owners: privacy + security. | Vault, encryption, deletion and receipts are modeled; life-safety preservation intentionally retains some sensitive material longer. | High |
| TM-013 | UX or automation communicates a false presence, false absence or rescue guarantee. | Ambiguous copy, color-only state, hidden missing sensor, alert pressure or derived score treated as fact. | Unsafe prioritization, abandoned search area, false expectation and operational error. | Prohibited labels, separate priority/confidence, visible coverage/missing sensors, signed external confirmation and comprehension gates. | P0-06 `terminal-ux`, Chromium smoke and 18 accessibility checks pass with zero false claims; human protocol remains P1a. Owners: product safety + operations. | No human participants, stress, language or manual accessibility evidence; misinterpretation remains possible. | High |
| TM-014 | Multi-bearer failover, raw bridging or downgrade creates loops, duplicate semantics or priority inversion. | Two or more adapters are active; policy trusts transport ACK/path, re-emits raw traffic or lacks stable message identity/TTL. | Airtime and battery exhaustion, SOS delay, duplicate dispatch and cross-bearer compromise. | `TransportProfile`, common signed message ID, per-path receipts, dedup, anti-loop, no raw bridge, authorized failover and SOS scheduling before interfaces. | P0-08 integrates three distinct bearer cells plus carry under partition with zero SOS priority inversion; physical coexistence remains unverified. Owners: communications + core security. | Independent bearers can fail together through shared spectrum, power, hardware or policy. | High |
| TM-015 | CSI sensing is repurposed for surveillance or person re-identification. | A CSI collector with phase or identifying features exists; temporal series are retained or correlated externally. BFI-based re-identification (~99.5% reported by KIT 2026) is demonstrated in literature. | Covert tracking of victims, responders or bystanders; legal exposure and loss of trust. | `csi-link-observation` consts `automatic_person_detection_allowed: false` and `silence_means_absence: false`, `max_declared_evidence: bench-validated`, amplitude-only declarations, mandatory `baseline_ref`, prohibited-label/identity red line, abstention and human-only fact creation. | Contract fixtures rejecting violating payloads; `rf-sensing-csi-review.md`; no runtime exists. Owners: privacy + sensing. | The physical phenomenon enables re-identification outside project controls; a third-party pipeline can bypass the contract boundary. | High |
| TM-016 | Passive RF metadata exposes or is forged to simulate persons. | A probe/BLE collector exists; raw MACs, SSID lists or payloads are retained, or an attacker injects crafted probes. | Privacy harm to victims/third parties; search diversion by spoofed phones; false absence under MAC randomization. | `passive-rf-observation` consts `pseudonym_scheme: incident_rotating_hmac`, `payload_retained: false`, `content_interception: false`, `active_emulation: false`; edge stripping; governed retention; low-confidence single-source fusion; Lifeseeker/Wi2SAR excluded as boundary-flagged external references. | Contract fixtures; `passive-rf-review.md`; legal line documented in `guides/regulatory.md`; no runtime exists. Owners: privacy + security. | Presence metadata is inherently sensitive; probe spoofing is not detectable by contract; jurisdiction review pending. | High |
| TM-017 | An SDR receiver is misused for field transmission or third-party demodulation. | TX-capable SDR hardware; a profile that does not pin receive-only; operator pressure during an incident. | Regulatory violation, harmful interference, unlawful content interception. | `sdr-receive-profile` consts `mode: receive_only_in_field` and `demodulate_third_party_traffic: false`; TX confined to `conducted_only` authorized bench; 406 MHz distress reception treated as public-interest passive reception coordinated with CAP/ICAO doctrine. | Contract fixtures; `sdr-receive-review.md`; no runtime exists. Owners: RF lead + security. | Contract cannot block TX-capable hardware; governance/SOP controls remain procedural. | Medium |
| TM-018 | A drone payload event injures people, corrupts sensing or shifts flight authority into the core. | Release without human confirmation; position estimate error; drop-sample states ingested into fusion; a bridge that can issue flight commands. | Physical injury, false candidates, loss of aircraft control. | `drone-deployment-event` consts `flight_authority_in_core: false` and `drop_sample_states_excluded_from_fusion: [RELEASED, IMPACT, SETTLING]`; human-confirmed `release_mode` with double-confirmation handshake; `node_position_estimate` and `drone_emi_baseline` events; geofence/lost-link documented; ADR-002. | Contract fixtures; `drone-geometry-review.md`; no runtime or flight integration exists. Owners: operations + safety. | Double confirmation is procedural; a misimplemented bridge could reintroduce command paths and needs its own gate. | High |
| TM-019 | RF quieting suppresses emergency communications or hides a victim's phone. | An isolation assembly encloses a sector carrying distress traffic or a possible victim; attenuation claims are unmeasured; isolation becomes a precondition for search. | Missed real distress, blind search sector, life-safety delay. | `rf-isolation-profile` consts `baseline_before_after_required: true` and `never_enclose_possible_victim_sector_without_analysis: true`; mandatory per-band measurements with uncertainty; announced window and operator record; independent emergency channel; alternatives ladder before isolation; `isolation_profile_ref` provenance; ADR-003. | Contract fixtures; `rf-quieting-review.md`; declared negative literature result (no SAR precedent); no runtime exists. Owners: RF lead + incident command. | The whole domain is `specified` with no published SAR evidence; procedural controls depend on discipline under stress. | High |
| TM-020 | The emergency autojoin AP is abused as a real evil twin, or third parties copy the design into a credential-harvesting portal. | AP hardware available; governance skipped; the published design reused without contract consts or portal labeling. | Credential compromise, unlawful interception, harm to victims/third parties and project trust collapse. | Activation validates only under the full `emergency_assumed_risk` lane (dual authorization, exact RF envelope/geofence, monitoring, expiry, stop condition, local kill switch); contract consts: no content inspection, no rerouting, no credential capture, no person identification, no default profile; portal visibly labeled as emergency; ADR-005. | Contract fixtures; `emergency-autojoin-review.md`; `guides/emergency-autojoin.md`; no runtime exists. Owners: security + incident command. | The technique is inherently dual-use; consts cannot stop third-party copies — real control is governance/legal, not technical. | High |
| TM-021 | A portal ACK or autojoin connection is misread as a located, alive or conscious person; fleet or third-party devices are read as victims. | UI/fusion treats connection as fact; fleet roster exclusion not applied; smartwatch/non-joining devices assumed covered. | Resource diversion, false life-safety confirmation, abandoned sectors. | Portal ACK never becomes `person_located` (contract const); mandatory human verification; fleet exclusion by roster; connection recorded as observation with uncertainty; efficacy declared `unverified` with a defined bench measurement experiment. | Contract fixtures; review `emergency-autojoin-review.md`; no runtime exists. Owners: operations + privacy. | Roster discipline under stress is procedural; low real-world autojoin rates may yield mostly false-positive traffic. | High |
| TM-022 | The autojoin AP perturbs scene networks or erodes the project's legal standing. | Activation without ICS-205 comms-plan coordination; jurisdiction without a clear distress exception; incomplete governance records. | Degraded rescue communications; legal exposure for operators and the whole repository. | ICS-205 coordination before activation; geofence and exact RF envelope; `emergency_assumed_risk` documented as never legal authorization; legal summary in `guides/regulatory.md`; ADR-005 review criteria (retire if measured efficacy ~0 or regulation changes). | Governance record per activation; legal review per jurisdiction remains external. Owners: incident command + legal/privacy. | Interception-adjacent interpretation varies by jurisdiction; reputational risk persists even under full governance. | Medium |

## Criticality calibration

No current vulnerability is rated Critical because the P0-08 integrated campaign composes deterministic simulations in-process and the repository does not contain an operational radio, beacon, identity, federation, SOS or human-terminal runtime. A future finding becomes Critical when a plausible path can create a false authenticated `operator.accepted`, systematically suppress or falsely confirm life-safety candidates, expose incident-wide keys across many cells, disable local kill switches, or cause broad harmful transmission with no effective containment.
