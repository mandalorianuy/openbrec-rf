...
Con el mapper de referencia lab-sim implementado (`openbrec/cot_export.py`) y el gate `interop-cot` ejecutado con receipt en `evidence/interop/`, el const `declared_evidence` del schema pasa de `specified` a `simulated`, alineando el contrato con la guía `docs/guides/ecosystem-integration.md` y `docs/research/sar-integration.md` (hallazgo de auditoría C4). El criterio es el mismo de los demás addons con mapper/gate: el **mapeo** está simulado en laboratorio; la **interop con ATAK real** sigue `unverified` y `evidence_level_elevation_by_export: false` se mantiene. Esta sección es un registro append-only: no modifica la decisión original, que queda conservada arriba.
