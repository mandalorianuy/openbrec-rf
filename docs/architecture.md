...
- **Offline finding** (`offline-finding-observation`, RFC-0002): detección pasiva de advertisements de redes de localización crowdsourced (Apple Find My, Google Find Hub, Samsung) como indicio débil — sin conexión GATT, sin identificación, sin retención de identificadores crudos, peso bajo en fusión y exclusión de la flota propia; nunca dispara alertas por sí solo.
- **AP de emergencia con auto-join** (`emergency-autojoin-profile`, ADR-005, RFC-0003): excepción gobernada — nunca por defecto — bajo el carril completo de `emergency_assumed_risk` (doble autorización, geofence, expiración, kill switch): un AP responde a cualquier SSID sondeado y el portal cautivo etiquetado convierte el teléfono de una víctima que no puede actuar en baliza (sonido, vibración, flash) con mensaje de emergencia; sin captura, sin rerouting, sin inspección de contenido, y el ACK del portal nunca es persona localizada; eficacia `unverified`.

Los estados de evidencia por tecnología y su base citable están en la [investigación SOTA](research/rf-sensing-state-of-the-art.md); las guías de dominio son [CSI](guides/csi-sensing.md), [RF pasiva](guides/passive-rf.md), [SDR](guides/sdr-beacons.md), [drones](guides/drone-geometry.md), [RF quieting](guides/rf-quieting.md) y [offline finding](guides/offline-finding.md). Tres dominios (CSI, RF pasiva, offline finding) tienen además simuladores de replay determinísticos (`openbrec/rf_sensing.py`, gates `rf-sensing-*`) que elevan sus addons a `simulated`. Ninguno tiene runtime de campo: son contratos, guías y replay, y todo collector futuro nace `unverified`.
